...
function query(filterBy) {
    ...
    if (!books || !books.length) {
        books = JSON.parse(JSON.stringify(gBooks))
...
function queryGoogleBooks(searchBy) {
    let googleBooks = _loadGoogleBooksFromStorage()
    if (!googleBooks || !googleBooks.length) {
        return fetch(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchBy}`)
            .then(response => response.json())
            .then(books => {
                _saveGoogleBooksToStorage(books)
                return books
            })
    }
...
function _saveGoogleBooksToStorage(books) {
    storageService.saveToStorage(GOOGLE_BOOKS_KEY, books)

}

function _loadGoogleBooksFromStorage() {
    return storageService.loadFromStorage(GOOGLE_BOOKS_KEY)
}
